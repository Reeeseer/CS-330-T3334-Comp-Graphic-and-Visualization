...
Cube::Cube(glm::vec3 position, glm::vec3 scale, const char* texture, Light keyLight, Light fillLight)
{
	Cube::position = position;
	Cube::texture = Texture(texture, GL_TEXTURE_2D, GL_TEXTURE0, GL_RGBA, GL_UNSIGNED_BYTE);
	Cube::keyLight = keyLight;
	Cube::fillLight = fillLight;
	Cube::shader = Shader("default.vert", "default.frag");

	Cube::vao = VAO();
	Cube::vbo = VBO(vertices, sizeof(vertices));
	Cube::ebo = EBO(indices, sizeof(indices));

	model = glm::translate(model, position);
	Cube::texture.texUnit(Cube::shader, "tex0", 0);
	model = glm::scale(model, scale);
}

Cube::Cube(glm::vec3 position, glm::vec3 scale, Shader shader, Light keyLight, Light fillLight)
{
	Cube::position = position;
	Cube::keyLight = keyLight;
	Cube::fillLight = fillLight;
...
void Cube::AssignUniforms()
{
	// Cube shader is activated to assign the key light and the filler light's position and color to do the math for the lighting.
	shader.Activate();
